...
for _ in range(TRIALS):
    tee_print(f"\rSalsa20-128:\t#{_+1}", end='')
    salsa20 = Salsa20.new(key=randbytes(16))
    salsa20.encrypt(buffer)
end = dt.now()
T = (end - start).total_seconds() / TRIALS
tee_print(f"\rSalsa20-128:\t{(end - start).total_seconds() / TRIALS:f} s\t{FS / T:f} MB/s")

start = dt.now()
for _ in range(TRIALS):
    tee_print(f"\rSalsa20-256:\t#{_+1}", end='')
    salsa20 = Salsa20.new(key=randbytes(32))
    salsa20.encrypt(buffer)
end = dt.now()
T = (end - start).total_seconds() / TRIALS
tee_print(f"\rSalsa20-256:\t{(end - start).total_seconds() / TRIALS:f} s\t{FS / T:f} MB/s")
